...
describe("PracticeWorkspace copy", () => {
  ...
  test("uses child-friendly practice selection naming and tab labels", () => {
    ...
    expect(screen.getByText("选一种方式，马上开始。")).toBeTruthy();
    ...
    expect(screen.getByRole("heading", { name: "今天想怎么练？" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "今日推荐 适合现在" }).getAttribute("aria-pressed")).toBe("true");
    ...
    expect(document.querySelectorAll(".practice-focus-card")).toHaveLength(1);
    fireEvent.click(screen.getByRole("button", { name: "巩固练习 打稳基础" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "挑战练习 试试进阶" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "今日推荐 适合现在" }));
    ...
    expect(screen.getByRole("heading", { name: "自己选练习" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "设置并开始" })).toBeTruthy();
    expect(screen.getByText("4×4｜入门｜1题")).toBeTruthy();
    expect(document.querySelector(".practice-garden-decor")).toBeNull();
